 ...
 * @author ankang
 */
public class ClientQueuePersistenceRheaKVSerializer {

    private static final Logger LOGGER = LoggerFactory.getLogger(ClientQueuePersistenceRheaKVSerializer.class);
    ...
    ClientQueuePersistenceRheaKVSerializer(@NotNull final PublishPayloadPersistence payloadPersistence) {
     ...
     */
    @NotNull
    byte[] serializeNewPublishKey(@NotNull final Key key) {
     ...
     */
    @NotNull
    byte[] serializeUnknownPubRelKey(@NotNull final Key key) {
    ...
    @NotNull byte[] serializeKey(@NotNull final Key key, final long number) {
     ...
     */
    @NotNull
    byte[] serializeKey(@NotNull final Key key) {
        final byte[] clientBytes = key.getQueueId().getBytes(UTF_8);
    ...
    int compareClientId(@NotNull final byte[] serializedClientId, @NotNull final byte[] serializedKey) {
        final int clientLength = serializedClientId.length;

        if (Arrays.compare(serializedClientId, Arrays.copyOfRange(serializedKey, 0, clientLength)) != 0) {
            return CLIENT_ID_NO_MATCH;
        }
        if (clientLength == (serializedKey.length - Long.BYTES)) {
    ...
    @NotNull
    Key deserializeKeyId(@NotNull final byte[] serializedKey) {
        final int clientIdLength = serializedKey.length - 1 - Long.BYTES;
        final String client = new String(serializedKey, 0, clientIdLength, UTF_8);
        final boolean shared = serializedKey[clientIdLength] == 1;
        return new Key(client, shared);
    }

    long deserializeIndex(@NotNull final byte[] serializedKey) {
        final int indexIndex = serializedKey.length - Long.BYTES;
        return Bytes.readLong(serializedKey, indexIndex);
    }

    // ********** Value **********

    @NotNull
    byte[] serializePublishWithoutPacketId(@NotNull final PUBLISH publish, final boolean retained) {
        return createPublishBytes(publish, retained);
    }

    @NotNull
    byte[] serializeAndSetPacketId(@NotNull final byte[] serializedValue, final int packetId) {
        final byte[] bytes = Arrays.copyOf(serializedValue, serializedValue.length);
        Bytes.copyUnsignedShortToByteArray(packetId, bytes, 0);
        return bytes;
    }

    @NotNull
    byte[] serializePubRel(@NotNull final PUBREL pubrel, final boolean retained) {
        return createPubrelBytes(pubrel.getPacketIdentifier(), retained, pubrel.getExpiryInterval(),
                pubrel.getPublishTimestamp());
    }

    int deserializePacketId(@NotNull final byte[] serializedValue) {
        return Bytes.readUnsignedShort(serializedValue, 0);
    }

    @NotNull
    MessageWithID deserializeValue(@NotNull final byte[] serializedValue) {

        if ((serializedValue[Short.BYTES] & PUBREL_BIT) == PUBREL_BIT) {
            final int packetId = Bytes.readUnsignedShort(serializedValue, 0);
            final PUBREL pubrel = new PUBREL(packetId);
            if (serializedValue.length >= Short.BYTES + 1 + Long.BYTES * 2) {
                final long expiry = Bytes.readLong(serializedValue, Short.BYTES + 1);
                pubrel.setExpiryInterval(expiry);
                final long timestamp = Bytes.readLong(serializedValue, Short.BYTES + 1 + Long.BYTES);
                pubrel.setPublishTimestamp(timestamp);
            }
            return pubrel;
        }
        if ((serializedValue[Short.BYTES] & PUBLISH_BIT) == PUBLISH_BIT) {
            return deserializePublish(serializedValue);
        }
    ...
    boolean deserializeRetained(@NotNull final byte[] serializedValue) {
        return (serializedValue[Short.BYTES] & RETAINED_MESSAGE_BIT) == RETAINED_MESSAGE_BIT;
    }
