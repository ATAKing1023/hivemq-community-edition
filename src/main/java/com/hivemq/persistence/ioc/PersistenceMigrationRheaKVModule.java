 ...
 * @author ankang
 */
class PersistenceMigrationRheaKVModule extends SingletonModule<Class<PersistenceMigrationRheaKVModule>> {
    ...
    PersistenceMigrationRheaKVModule() {
        super(PersistenceMigrationRheaKVModule.class);
    ...
    protected void configure() {
        if (retainedPersistenceType == PersistenceType.FILE_DISTRIBUTED) {
            bind(RetainedMessageLocalPersistence.class).to(RetainedMessageRheaKVLocalPersistence.class)
                    .in(Singleton.class);
        }

        if (payloadPersistenceType == PersistenceType.FILE_DISTRIBUTED) {
            bind(PublishPayloadLocalPersistence.class).to(PublishPayloadRheaKVLocalPersistence.class)
                    .in(Singleton.class);
        }
    }
}
